package com.gdkdemo.location.service;
...
import com.gdkdemo.location.LocationDemoActivity;
import com.gdkdemo.location.R;
...
// ...
public class LocationDemoLocalService extends Service
{
...
//            wl.release();
    ...
    public class LocalBinder extends Binder {
        public LocationDemoLocalService getService() {
            return LocationDemoLocalService.this;
        }
    }
...
//                 liveCard.setNonSilent(true);       // for testing.
    ...
    private void initializeLocationManager()
    {
        ...
        for (String provider : providers) {
            if (locationManager.isProviderEnabled(provider)) {
                if(Log.D) Log.d("Location provider added: provider = " + provider);
            } else {
                if(Log.I) Log.i("Location provider not enabled: " + provider);
            }
            ...
            try {
                locationManager.requestLocationUpdates(provider, 5000L, 5.0f, new LocationListener() {
                    ...
                    public void onLocationChanged(Location location) {
                        if(Log.D) Log.d("locationChanged: location = " + location);
                        processLocationData(location);
                    }
                    @Override
                    public void onProviderDisabled(String provider) {
                        if(Log.I) Log.i("providerDisabled: provider = " + provider);
                        // TBD
                    }
                    @Override
                    public void onProviderEnabled(String provider) {
                        if(Log.I) Log.i("providerEnabled: provider = " + provider);
                        // TBD
                    }
                    @Override
                    public void onStatusChanged(String provider, int status, Bundle extras) {
                        if(Log.I) Log.i("statusChanged: provider = " + provider + "; status = " + status + "; extras = " + extras);
                        // TBD
                    }
                });
            } catch (Exception e) {
                // ignore
                Log.w("requestLocationUpdates() failed for provider = " + provider);
            }
        }
    }
    ...
    private void processLocationData(Location location)
    {
        long now = System.currentTimeMillis();
        if(location == null) {
            Log.d("processLocationData() called with null location");
            // TBD:
            // periodic checking???
            ...
            if(acc < lastAcc) {
                ...
                if(distance >= acc * 0.001 ) {
                    ...
                    if(Log.D) Log.d("New data is within the error bar from the last location: distance = " + distance + "; new location accuracy = " + acc);
                }
            }
        }
    ...
    {
        Log.d("startHeartBeat() called.");

        AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        Intent i = new Intent(context, LocationDemoLocalService.class);
    ...
    {
        Log.d("stopHeartBeat() called.");

        Intent intent = new Intent(context, LocationDemoLocalService.class);
